...
## Couverture des Menaces de l'OWASP Top 10
...
## Hardening DevSecOps et Sécurité de l'Infrastructure
...
- Elle est intégrée dans un sous-réseau délégué (_MySQL Subnet_) au sein d'un réseau virtuel privé Azure (VNet).
...
- Tout trafic entrant (_Ingress_) est bloqué par défaut pour les conteneurs `backend`, `auth` et `client`.
