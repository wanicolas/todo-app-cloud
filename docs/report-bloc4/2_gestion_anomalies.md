## Processus de Collecte et Consignation (C4.2.1)
...
## Traitement et Déploiement du Correctif (C4.2.2)

### Élaboration et Code du Correctif
...
### Analyse DevSecOps et Perspective Pérenne (Refresh Tokens)

L'extension de la durée de vie du JWT à 12h constitue un **Hotfix d'urgence** immédiatement efficace pour stopper la dégradation de l'expérience utilisateur.

Cependant, d'un point de vue DevSecOps, conserver un jeton _stateless_ valide pendant 12h sans possibilité de révocation centralisée augmente la surface d'exposition en cas de vol de jeton.

Pour corriger l'anomalie de manière pérenne et alignée avec les meilleures pratiques de sécurité, l'équipe a consigné dans le backlog d'amélioration l'évolution vers une architecture **Double Jetons (OAuth2 / OIDC)** :

- **Access Token court (15 min) :** Transmis dans les en-têtes `Authorization: Bearer` pour authentifier les requêtes API.
- **Refresh Token long (7 jours) :** Stocké côté client dans un cookie sécurisé non accessible en JavaScript (`HttpOnly`, `Secure`, `SameSite=Strict`), utilisé pour renouveler silencieusement l'Access Token en arrière-plan sans interrompre la session utilisateur.
